...
namespace Filament\Pages\Auth\Company;
...
class Profile extends Page
{
    ...
    protected function getAvatarFormComponent(): Component
    {
        return FileUpload::make('avatar')
            ->label('تصویر')
            ->disk('public')
            ->directory('companies/avatar')
            ->visibility('private')
            ->image()
            ->imageEditor()
            ->maxSize(1024)
            ->hint('حداکثر 1 مگابایت')
            ->hintColor('success')
    ...
    protected function getFirstnameFormComponent(): Component
    {
        return TextInput::make('firstname')
            ->label(__('filament-panels::pages/auth/edit-profile.form.firstname.label'))
            ->required()
            ->maxLength(255)
            ->autofocus();
    }
    protected function getLastnameFormComponent(): Component
    {
        return TextInput::make('lastname')
            ->label(__('filament-panels::pages/auth/edit-profile.form.lastname.label'))
            ->required()
            ->maxLength(255)
            ->autofocus();
    }
    protected function getCompanyNameFormComponent(): Component
    {
        return TextInput::make('company_name')
            ->label(__('filament-panels::pages/auth/edit-profile.form.company_name.label'))
            ->maxLength(255)
            ->autofocus();
    }
    ...
    protected function getMobileFormComponent(): Component
    {
        return TextInput::make('mobile')
            ->label(__('filament-panels::pages/auth/edit-profile.form.mobile.label'))
            ->required()
            ->disabled()
            ->maxLength(255)
    ...
    protected function getPasswordFormComponent(): Component
    {
        return TextInput::make('password')
            ...
            ->afterStateHydrated(function ($state, $set) {
                if ($state) {
                    // به‌صورت شرطی فیلد را غیرفعال کنید
                    $set('password', null);
                }
            });
    }
