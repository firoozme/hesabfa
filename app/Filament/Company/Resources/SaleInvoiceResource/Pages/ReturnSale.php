...
class ReturnSale extends Page implements HasForms
{
    ...
    protected static ?string $title = 'برگشت فروش';
        ...
        try {
            $invoice = DB::transaction(function () use ($data) {
                ...
                $invoice = Invoice::create([
                    'name' => 'برگشت فروش '. $this->invoice->number,
                    'number' => $data['number'],
                    'date' => $data['date'],
                ...
                // ]);
                $storeTransaction = StoreTransaction::withoutEvents(function () use ($invoice) {
                ...
                foreach ($data['items'] as $item) {
                    StoreTransactionItem::withoutEvents(function () use ($invoice, $storeTransaction, $item) {
                        return StoreTransactionItem::create([
                    ...
                    Log::warning('Product inventory:'. $product->inventory);

                    // چون قبلا store_product در ایجاد شده پس دیگر نیازی به ایجاد مقدار جدید نیست
                    InventoryService::updateInventory($product, $invoice->store, $item['quantity'], 'entry');
                }
