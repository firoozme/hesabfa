...
use App\Filament\Company\Resources\PurchaseReturnInvoiceResource\Pages;
use App\Filament\Company\Resources\PurchaseReturnInvoiceResource\RelationManagers;

class PurchaseReturnInvoiceResource extends Resource
{
    protected static ?string $model = Invoice::class;
    protected static ?string $navigationLabel = 'فاکتور برگشت خرید';
    protected static ?string $pluralLabel = 'فاکتورهای برگشت خرید';
    protected static ?string $label = 'فاکتور برگشت خرید';
                ...
                Grid::make(['default' => 3])
                    ->schema([
                        ...
                        Forms\Components\Select::make('store_id')
                            ->label('انبار')
                            ->relationship('store', 'title', fn($query) => $query->whereNull('deleted_at'))
                            ->options(Store::where('company_id', auth()->user('company')->id)->whereNull('deleted_at')->pluck('title', 'id'))
                            ->required()
                            ->live()
                            ->suffixAction(
                                Act::make('add_store')
                                    ...
                                    ->rules([
                                        function (Get $get, $record) {
                                            return function (string $attribute, $value, Closure $fail) use ($get, $record) {
                                                ...
                                                $type = $get('../../type');
                                                if ($type === 'purchase_return' && !$record) { // فقط موقع ایجاد چک می‌کنیم
                                                    ...
                                                    if ($store) {
                                                        ...
                                                        if ($currentStock < $quantity) {
                                                            $fail("موجودی کافی برای برگشت محصول در انبار وجود ندارد.");
                                                        }
                                                    }
                                                }
                                            };
        ...
        return $table
            ...
            ->actions([
                ...
                Action::make('payments')
                    ->label('پرداخت‌ها')
                    ->url(fn(Model $record): string => route('filament.company.resources.return-purchase-invoices.payments', ['record' => $record]))
    ...
    {
        return [
            'index' => Pages\ListPurchaseReturnInvoices::route('/'),
            'create' => Pages\CreatePurchaseReturnInvoice::route('/create'),
            'edit' => Pages\EditPurchaseReturnInvoice::route('/{record}/edit'),
        ];
    }
