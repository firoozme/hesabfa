...
class Receipts extends Page implements Tables\Contracts\HasTable
{
    use Tables\Concerns\InteractsWithTable;

    protected static string $resource = InvoiceResource::class;
    protected static string $view = 'filament.company.resources.invoice-resource.pages.receipts';
                ...
                EditAction::make()
                    ...
                    ->action(function (array $data, Payment $record) {
                        ...
                        if ($data['paymentable_type'] === 'App\Models\Check') {
                            ...
                            // اگر روش پرداخت تغییر کرده، مبلغ قبلی را به حساب قبلی برگردانید
                            if ($originalPaymentableType !== $data['paymentable_type'] || $originalPaymentableId !== $data['paymentable_id']) {
                                if ($originalPaymentableType !== 'App\Models\Check') {
                                    ...
                                    $originalAccount->outgoingTransfers()->create([
                ...
                Action::make('add_receipt')
                                    ...
                                    TextInput::make('amount')
                                        ...
                                        ->rules([
                                            fn(Get $get) => function (string $attribute, $value, callable $fail) use ($get) {
                                                ...
                                                $totalPaid = collect($get('../../receipts'))->sum(fn($item) => self::cleanNumber($item['amount'] ?? 0));
                    ...
                    ->action(function (array $data) {
                        ...
                        foreach ($data['receipts'] as $receiptData) {
                            $receiptData['amount'] = self::cleanNumber($receiptData['amount']);
                            $totalPaid += $receiptData['amount'];

                            if ($receiptData['paymentable_type'] === 'App\Models\Check') {
                                $check = Check::create([
                                ...
                                $receiptData['paymentable_id'] = $check->id;
                            } else {
                                $account = $receiptData['paymentable_type']::findOrFail($receiptData['paymentable_id']);
                                // $account->incomingTransfers()->create([
                                //     'amount' => $receiptData['amount'],
                            ...
                            Payment::create([
                                ...
                                'paymentable_type' => $receiptData['paymentable_type'],
                                'paymentable_id' => $receiptData['paymentable_id'],
                                'amount' => $receiptData['amount'],
