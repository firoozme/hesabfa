...
class EditPurchaseReturn extends Page implements HasForms
{
    use InteractsWithForms;

    protected static string $resource = InvoiceResource::class;
    protected static string $view = 'filament.company.resources.invoice-resource.pages.edit-return-purchase';
    ...
    public function mount($record)
    {
        ...
        if (!$this->invoice || $this->invoice->type !== 'purchase_return') {
            Notification::make()
                ->title('فاکتور برگشت خرید یافت نشد')
                ->danger()
                ->send();
                        ...
                        Radio::make('accounting_auto')
                            ...
                            ->afterStateUpdated(
                                function ($state, callable $set) {
                                    $invoice = Invoice::where('type', 'purchase_return')->withTrashed()->orderBy('number', 'desc')->first();
    ...
    protected function getFormActions(): array
    {
        return [
            ...
            Action::make('updateAndDownloadPdf')
                ...
                ->modalDescription('آیا می‌خواهید فاکتور برگشت خرید را به‌روزرسانی کرده و فایل PDF آن را دانلود کنید؟')
    ...
    public function submit()
    {
        ...
        try {
            $invoice = DB::transaction(function () use ($data) {
                ...
                $storeTransaction = StoreTransaction::where('reference', 'PR-' . $this->invoice->number)->first();
                if ($storeTransaction) {
                ...
                $storeTransaction = StoreTransaction::create([
                    ...
                    'type' => 'exit',
                    'date' => $this->invoice->date,
                    'reference' => 'PR-' . $this->invoice->number,
                ]);
                ...
                foreach ($data['items'] as $item) {
                    ...
                    InventoryService::updateInventory($product, $this->invoice->store, -$item['quantity'], 'exit');
                }
            ...
            Notification::make()
                ->title('فاکتور برگشت خرید با موفقیت به‌روزرسانی شد')
                ->success()
                ->send();
            ...
            Notification::make()
                ->title('خطا در به‌روزرسانی فاکتور برگشت خرید')
                ->body($e->getMessage())
                ->danger()
                ->send();
            return;
