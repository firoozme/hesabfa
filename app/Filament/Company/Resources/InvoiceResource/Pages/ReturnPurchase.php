...
class ReturnPurchase extends Page implements HasForms
{
    use InteractsWithForms;
    protected static string $resource = InvoiceResource::class;
    protected static ?string $title = 'برگشت خرید';

    protected static string $view = 'filament.company.resources.invoice-resource.pages.return-purchase';
    ...
    public function mount($record)
    {
        ...
        if (!$this->invoice || $this->invoice->type !== 'purchase') {
            Notification::make()
                    ->title('فاکتور خرید یافت نشد')
                    ->body($e->getMessage())
                    ->danger()
                    ->send();
                return;
    ...
    protected function getFormActions(): array
    {
        return [
            ...
            //     ->modalDescription('آیا می‌خواهید فاکتور برگشت خرید را ایجاد کرده و فایل PDF آن را دانلود کنید؟')
    ...
    public function submit()
    {
        ...
        try {
            $invoice = DB::transaction(function () use ($data) {
                ...
                // ایجاد فاکتور برگشت خرید
                $invoice = Invoice::create([
                    'name' => 'برگشت خرید '.$this->invoice->name,
                    'number' => $data['number'],
                    'date' => $data['date'],
                ...
                foreach ($data['items'] as $item) {
                    $invoice->items()->create([
                        ...
                        'total_price' => $item['quantity'] * (float) str_replace(',', '', $item['unit_price']),
                    ]);
                }
                ...
                foreach ($data['items'] as $item) {
                    ...
                    InventoryService::updateInventory($product, $invoice->store, $item['quantity'], 'nochange');
                }
            ...
            Notification::make()
                ->title('فاکتور برگشت خرید با موفقیت ایجاد شد')
                ->success()
                ->send();
            ...
            Notification::make()
                ->title('خطا در ایجاد فاکتور برگشت خرید')
                ->body($e->getMessage())
                ->danger()
                ->send();
            return;
